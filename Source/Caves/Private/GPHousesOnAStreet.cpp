...
void UGPHousesOnAStreet::GenerateLevel() {
	...
	for(int i = 0; i < 2; i++){
		...
		parent->SetTile(cursor_x, cursor_y, parent->floor_info->floor_material, 32, true);
	}
	...
	for (int i = 0; i < house_locations.Num(); i++) {
		...
		parent->SetTile(house_location[0], house_location[1], parent->floor_info->wall_material, 8, false);
		parent->SetTile(other_house_location[0], other_house_location[1], parent->floor_info->wall_material, 8, false);
