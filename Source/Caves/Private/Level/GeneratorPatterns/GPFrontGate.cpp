...
void UGPFrontGate::GenerateLevel() {
	...
	for (int i = 0; i < 20; i++) {
		...
		if (parent->GetTile(cursor_x + (perp[0] * wall_thickness), cursor_y + (perp[1] * wall_thickness))->GetTileIndex() == DefaultVoidMaterial()) { parent->SetTile(cursor_x, cursor_y, DefaultWallMaterial(), wall_thickness, false); end_0 = { cursor_x + (perp[0]) , cursor_y + (perp[1]) }; break; }


		parent->SetTile(cursor_x, cursor_y, MaterialType::rusty_metal, wall_thickness, false);
		cursor_x += perp[0];
		cursor_y += perp[1];
	}
	...
	for (int i = 0; i < 20; i++) {
		...
		if (parent->GetTile(cursor_x + (-perp[0] * wall_thickness), cursor_y + (-perp[1] * wall_thickness))->GetTileIndex() == DefaultVoidMaterial()) { parent->SetTile(cursor_x, cursor_y, DefaultWallMaterial(), wall_thickness, false); end_1 = { cursor_x + (-perp[0]) , cursor_y + (-perp[1]) }; break; }
